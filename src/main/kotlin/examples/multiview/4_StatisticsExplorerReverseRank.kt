...
fun main() = application {
    ...
    program {
        ...
        val memesGrouped = memes.groupBy { it.year.toString() }.filterValues { it.size > 1 }
        ...
        val groupGrid = GridView(groupStatistics, 1, 256.0, 64.0) { (name, statistics), state ->
            ...
            drawer.text(" ${name?:"no name"} (${memesGrouped[name]!!.size})", 20.0, 20.0)
        }
        ...
        groupGrid.activeItemChanged.listen {
            ...
            statisticsGrid.activeItem = statisticsGrid.items[0]
        }
