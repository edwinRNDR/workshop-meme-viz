...
fun main() = application {
    ...
    program {
        ...
        statisticsGrid.activeItemChanged.listen { item ->
            ...
            val unrankedPositions = List(memes.size) { index ->
                Pair(index, mapper(features[activeFeature]!![index]))
            }
            val rankedPositions = unrankedPositions.sortedByDescending { it.second }
            val newPositions = MutableList(memes.size) { Vector2.ZERO }

            val newSize = MutableList(memes.size) { 0.0 }
            val memesPerRow = 16

            for ((rank, pos) in rankedPositions.withIndex()) {
                val x = rank.mod(memesPerRow)
                val y = rank / memesPerRow
                newPositions[pos.first] = Vector2(x * 32.0, y * 32.0)
                if (rank < 100) {
                    newSize[pos.first] = 1.0
                }
            }


            scatterPlot.positions = newPositions
            scatterPlot.sizes = newSize
